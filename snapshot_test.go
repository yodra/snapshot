...
func TestCreateSnaps(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.Name, func(t *testing.T) {
			labelTest := "creating snaps"
			c.Assert(t, labelTest, []byte(tc.Out))

			recv, err := ioutil.ReadFile(path.Join(tmpdir, getSnapFilename(t.Name()+labelTest, ".snap")))
			if err != nil {
				t.Fatalf("Expected snap file %s to be created but does not exist", getSnapFilename(t.Name()+labelTest, ".snap"))
			}
...
func TestSnaps(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.Name, func(t *testing.T) {
			labelTest := "check created snaps"
			if err := os.Unsetenv("UPDATE_SNAPSHOTS"); err != nil {
				t.Fatalf("Unexpected error unsetting environment: %s", err)
			}
			if err := createSnapshot(t.Name()+labelTest, []byte(tc.Out), c.Directory, c.Extension); err != nil {
			...
			c.Assert(t, labelTest, []byte(tc.Out+"\nupdated"))
			recv, err := ioutil.ReadFile(path.Join(c.Directory, getSnapFilename(t.Name()+labelTest, ".snap")))
			if err != nil {
